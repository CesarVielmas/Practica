...
int main(int argc, char * argv[])
{
  ...
  int Maximo = Distancias[nodoArbitrario][1];
  i = 0;
  int j = 0;
  int p = CantidadDeFilas -2;
  int VariableTour,NumeroCliente;
  int Costo[CantidadDeFilas+1];
  Costo[0] = Maximo;
  while (CantidadDeFilas > i+1)
  {
    while (CantidadDeFilas > j)
    {
      if (Distancias[nodoArbitrario][j+1] > Maximo)
      {
        Maximo = Distancias[nodoArbitrario][j+1];
        Costo[i+1] = Maximo;
      }
      else
      {
    ...
    while (j>0)
    {
      if (Distancias[nodoArbitrario][j]== Maximo)
      {
        NumeroCliente = j;
        j = 0;
      }
    ...
    if (i == 0)
    {
       printf("T = %i-",nodoArbitrario+1);
       j = 0;
      while (CantidadDeFilas > j )
      {
        Distancias[j][nodoArbitrario+1] = 0;
        j++;
      }
    }
    else
    {
      ...
      while (CantidadDeFilas > j )
      {
        Distancias[j][NumeroCliente] = 0;
        j++;
      }
    j = 0;
    i++;
    nodoArbitrario = NumeroCliente-1;
    Maximo = Distancias[nodoArbitrario][1];
  }
  int CostoTotal = 0;
  int CostoTotal1,CostoTotal2;
  printf("%i",nodoArbitrario1);
  Costo[CantidadDeFilas] = Distancias[nodoArbitrario][nodoArbitrario1];
  for (int m = 0; m <= CantidadDeFilas; m++)
  {
    if (CantidadDeFilas%2 > 0 )
    {
    CostoTotal1 = Costo[m] + Costo[m+1];
    CostoTotal = CostoTotal1 + CostoTotal;
    }
    else
    {
      if (m == CantidadDeFilas-2)
      {
    CostoTotal1 = Costo[m] + Costo[m+1];
    CostoTotal2 = Costo[m+2];
    CostoTotal = CostoTotal1 + CostoTotal + CostoTotal2;
      }
      else
      {
    CostoTotal1 = Costo[m] + Costo[m+1];
  ...
  if (CantidadDeFilas%2 > 0)
  {
    CostoTotal = CostoTotal - Costo[CantidadDeFilas-1];
  }
  else
  {
